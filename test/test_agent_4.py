...
while not flag_stop:
    ...
    human_input_string = input("Please ask the agent a question: ")

    if human_input_string == 'STOP':
        flag_stop = True
        continue

    result = agent_executor.invoke({"input": human_input_string, "chat_history": chat_history})

    chat_history.extend(
        [
            HumanMessage(content=human_input_string),
